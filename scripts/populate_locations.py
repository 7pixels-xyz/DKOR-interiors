      ...
      <section className="relative h-[70vh] flex items-center justify-center pt-24 px-6 md:px-12 overflow-hidden bg-[#1A1A1A]">
        <div className="absolute inset-0 z-0">
          <img 
            ...
            alt="Luxury Interior Design in {name}"
        ...
        >
          ...
          <h1 className="font-serif text-5xl md:text-7xl text-white leading-tight">
            Luxury Interior Design in {name}
          </h1>
          <p className="text-white/70 text-lg font-light leading-relaxed max-w-2xl mx-auto">
            DKOR Interiors brings award-winning architectural design and uncompromising material integrity to high-end residential and commercial properties throughout {name}.
          </p>
        </motion.div>
      </section>
      ...
      <section className="py-24 px-6 md:px-12 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          <motion.div 
            ...
            className="space-y-8"
          >
            <h2 className="font-serif text-4xl text-[#1A1A1A]">Elevating {name} Real Estate</h2>
            <p className="text-[#555555] font-light leading-relaxed text-lg">
              Whether you are renovating a coastal penthouse, building a custom estate, or redesigning a commercial workspace, our process is designed to handle the complexity of luxury builds in {name}. 
            </p>
            <p className="text-[#555555] font-light leading-relaxed text-lg">
...
for loc in locations:
    content = template.replace("{name}", loc["name"])
    path_dir = os.path.join(os.getcwd(), "app", "locations", loc["slug"])
    os.makedirs(path_dir, exist_ok=True)
    with open(os.path.join(path_dir, "page.tsx"), "w", encoding="utf-8") as f:
        f.write(content)

all_locations = r'''"use client";
...
with open(os.path.join(os.getcwd(), "app", "locations", "page.tsx"), "w", encoding="utf-8") as f:
    f.write(all_locations)
