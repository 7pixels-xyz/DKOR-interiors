...
export default function ContactPage() {
  ...
  return (
    <div className="bg-[#FFFFFF] min-h-screen">
      ...
      <div className="pb-24 pt-16 px-6 md:px-12 max-w-7xl mx-auto space-y-20 relative z-20">
            ...
            <div className="bg-[#1A1A1A] text-white p-10 md:p-14 rounded-[2rem] space-y-12 shadow-2xl">
              ...
              <div className="space-y-8 text-sm">
                {[
                  ...
                  >
                    <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center shrink-0 group-hover:bg-[#5C6B57] transition-colors duration-500">
                      <item.icon className="w-4 h-4 text-white group-hover:scale-110 transition-transform duration-500" />
                    </div>
                    <div className="pt-1">
                      <p className="font-mono tracking-widest uppercase text-xs text-[#5C6B57] mb-1">{item.label}</p>
                      {item.link ? (
                        <a href={item.link} target={item.label === "Instant Messaging" ? "_blank" : undefined} rel="noopener noreferrer" className="text-white/80 font-light block hover:text-white transition-colors whitespace-pre-line">
                          {item.text}
                        </a>
                      ) : (
                        <p className="text-white/80 font-light leading-relaxed whitespace-pre-line">{item.text}</p>
                      )}
                    </div>
                  </motion.div>
                ))}
              ...
              {submitted ? (
                ...
                >
                  <motion.div 
                    ...
                    className="w-20 h-20 bg-[#1A1A1A] text-[#5C6B57] rounded-full flex items-center justify-center mx-auto mb-6"
                  >
                ...
                <>
                  <div className="space-y-8">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                      ...
                      >
                        ...
                        <input
                          ...
                          className="w-full px-5 py-4 bg-[#E6E8E3] rounded-xl border-2 border-transparent focus:bg-white focus:border-[#1A1A1A] outline-none text-sm transition-all placeholder:text-neutral-400 hover:border-neutral-300"
                        />
                      </motion.div>
                      <motion.div 
                        initial={{ opacity: 0, y: 20 }}
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {scopeOptions.map((opt) => (
                          <button
                            ...
                            className={`py-4 px-4 text-xs font-mono tracking-wider rounded-xl border-2 transition-all duration-500 ${
                              scope === opt 
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                      ...
                      >
                        ...
                        <select
                          required
                          className="w-full px-5 py-4 bg-[#E6E8E3] rounded-xl border-2 border-transparent focus:bg-white focus:border-[#1A1A1A] outline-none text-sm transition-all text-neutral-600 appearance-none hover:border-neutral-300"
                        >
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {budgetOptions.map((opt) => (
                          <button
                            ...
                            className={`py-4 px-2 text-[11px] font-mono tracking-widest rounded-xl border-2 transition-all duration-500 ${
                              budget === opt 
                    ...
                    >
                      ...
                      <textarea
                        ...
                        className="w-full px-5 py-4 bg-[#E6E8E3] rounded-xl border-2 border-transparent focus:bg-white focus:border-[#1A1A1A] outline-none text-sm transition-all resize-none placeholder:text-neutral-400 leading-relaxed hover:border-neutral-300"
                      />
                    </motion.div>
                  </div>
                  ...
                  >
                    <button
                      type="submit"
                      className="group w-full py-5 bg-[#1A1A1A] text-white rounded-xl text-xs uppercase tracking-[0.2em] font-mono hover:bg-[#5C6B57] transition-all duration-500 flex items-center justify-center gap-4 relative overflow-hidden"
                    >
